...
pub async fn get_assets_by_original_path(configuration: &configuration::Configuration, path: &str) -> Result<Vec<models::AssetResponseDto>, Error<GetAssetsByOriginalPathError>> {
    ...
    let uri_str = format!("{}/view/folder", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetAssetsByOriginalPathError> = serde_json::from_str(&content).ok();
...
pub async fn get_unique_original_paths(configuration: &configuration::Configuration, ) -> Result<Vec<String>, Error<GetUniqueOriginalPathsError>> {
    ...
    let uri_str = format!("{}/view/folder/unique-paths", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUniqueOriginalPathsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
